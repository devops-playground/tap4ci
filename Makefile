...
# $(info $$DOCKER_USERNAME is [${DOCKER_USERNAME}])
# $(info $$DOCKER_PASSWORD is [${DOCKER_PASSWORD}])
...
define docker_run
	( \
		...
		pattern=$(shell printf '%s' "$(DOCKER_PASSWORD)" | sed -e 's/\//\\/g') ; \
